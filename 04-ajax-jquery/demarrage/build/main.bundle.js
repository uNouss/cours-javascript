  ...
  _createClass(AddPizzaPage, [{
    ...
    value: function submit(event) {
      ...
      if (errors.length) {
        ...
        console.log("Pizza \"".concat(values.nom, "\" enregistr\xE9e avec succ\xE8s !")); // si il n'y a pas d'erreur, on appelle le webservice post /api/v1/pizzas avec les valeurs saisie

        var pizza = {
          "nom": "".concat(values.nom),
          "base": "".concat(values.base),
...
function renderHome(data) {
  ...
  jquery__WEBPACK_IMPORTED_MODULE_0___default()('.pizzasContainer').removeClass('is-loading');
}

fetch('./news.html').then(function (response) {
  return response.text();
}).then(displayNews); //console.log(2);

jquery__WEBPACK_IMPORTED_MODULE_0___default()('.pizzasContainer').addClass('is-loading');
fetch('http://localhost:8080/api/v1/pizzas').then(function (response) {
  return response.json();
}).then(renderHome); // console.log( $('a.navbar-brand') );
// $('a').html("jQuery forever");
// console.log( $('a.navbar-brand').html() );

jquery__WEBPACK_IMPORTED_MODULE_0___default()('a.navbar-brand img').on('click', function (event) {
  event.preventDefault();
  _PageRenderer_js__WEBPACK_IMPORTED_MODULE_2__["default"].renderPage(homePage);
});
jquery__WEBPACK_IMPORTED_MODULE_0___default()('.pizzaListButton').on('click', function (event) {
  event.preventDefault();
  _PageRenderer_js__WEBPACK_IMPORTED_MODULE_2__["default"].renderPage(homePage);
});
jquery__WEBPACK_IMPORTED_MODULE_0___default()('article.media img').click(function (event) {
  event.preventDefault();
  console.log("salut");
});
